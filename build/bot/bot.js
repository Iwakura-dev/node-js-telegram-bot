...
import { author, greeting, help } from '../constants/text/text.js';
import { Commands } from '../menu/commands/commands.js';
import { inlineKeyboardVacancy, inlineUnsubscribeKeyboard, rootKeyboard, } from '../menu/menu.js';
import { sendHHVacancies } from '../utils/utils.js';
export const bot = new Bot(constants.token, {
    botInfo: {
        id: 6871508134,
        is_bot: true,
...
bot.command('settings', async (ctx) => {
    if (ctx.from?.id) {
        ...
        if (userSubscriptions[userId] && userSubscriptions[userId].length > 0) {
            ...
            await ctx.reply('<b>Вы не можете просматривать свой профиль, выберите свое направление!</b>', {
                parse_mode: 'HTML',
            });
        }
    }
...
bot.command('unsubscribe', async (ctx) => {
    if (ctx.from?.id) {
        ...
        if (!userSubscriptions[userId]) {
            await ctx.reply('<b>Для начала подпишетесь на направление, чтобы в будущем отписаться от него!</b>', {
                parse_mode: 'HTML',
            });
        }
        else {
...
bot.on(':text', async (ctx) => {
    switch (ctx.msg?.text) {
        case 'Направление':
            await ctx.reply('<b>Выберите из списка направления в котором вы заинтересованы!</b>', {
                reply_markup: inlineKeyboardVacancy,
                parse_mode: 'HTML',
            });
            break;
        case 'Донат':
            await ctx.reply(`<b>Если ты хочешь поддержать автора данного проекта, автор будет рад угоститься кружечкой горячего кофе</b>\n\nBuy Me A Coffee - ${constants.buyMeCoffe}\n\nKo-Fi - ${constants.ko_fi}`, {
                parse_mode: 'HTML',
            });
            break;
        case 'Об авторе':
...
bot.callbackQuery(['subscribe_frontend', 'subscribe_backend', 'subscribe_fullstack'], async (ctx) => {
    const userId = ctx.from.id;
    if (!userSubscriptions[userId]) {
...
bot.callbackQuery(['unsubscribe_frontend', 'unsubscribe_backend', 'unsubscribe_fullstack'], async (ctx) => {
    if (ctx.chat?.id) {
